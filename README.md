...
SQLite Viewer 是一个面向 Windows 的 SQLite 桌面查看器，支持安全浏览 SQLite 数据库，并可在 Data 页面新增、编辑和删除单条表记录。

## 主要功能

- 打开 SQLite 数据库并浏览表、视图、索引和数据。
- 在 Data 页面新增、编辑和删除单条表记录；删除前必须确认。
...
应用允许通过 Data 页面修改表记录，但 SQL 页面仍限制为只读：

1. Data 页面仅支持新增、编辑和删除单条记录；删除必须经过确认。
2. SQL 输入会进行白名单校验，只允许安全的只读语句。

因此，导出 CSV、浏览结构和执行 SQL 查询均不会修改源数据库。对于损坏、截断、缺失或非 SQLite 的文件，应用会显示打开失败信息。
...
- 在左侧对象树中选择表、视图或索引。
- 使用 **Data** 页面浏览表数据；可使用上一页、下一页进行翻页。选择一条记录后可编辑或删除；删除前会要求确认。
...
- Data 页面仅支持新增、编辑和删除单条表记录；不支持批量删除、删除表或撤销删除。
